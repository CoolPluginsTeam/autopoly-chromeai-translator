...
const UpdateClassicPage = (props) => {
    ...
    const postDataUpdate = () => {
        ...
        if(atfp_global_object.slug_translation_option === 'slug_translate' || atfp_global_object.slug_translation_option === 'slug_keep'){
            let translateContent = '';
            if(atfp_global_object.slug_translation_option === 'slug_translate'){
                translateContent = select('block-atfp/translate').getTranslatedString('slug', postContent.slug_name, null, service);
            }
        ...
        if(postContent.title && postContent.title.trim() !== '') {
            const translateContent = select('block-atfp/translate').getTranslatedString('title', postContent.title, null, service);
        ...
        if(postContent.excerpt && postContent.excerpt.trim() !== '') { 
            const translateContent = select('block-atfp/translate').getTranslatedString('excerpt', postContent.excerpt, null, service);
            const excerptBox = document.querySelector('#postexcerpt.postbox textarea#excerpt');
            if(excerptBox) {
                excerptBox.value = translateContent;
            }

            if(atfp_global_object.post_type =='product' && window.tinymce){
                ...
                if(value && '' !== value && Object.keys(AllowedMetaFields).includes(value)){
                    ...
                    if(metaId && '' !== metaId){
                        ...
                        if(valueInputField && valueInputField.value ){
                            const translatedValue=select('block-atfp/translate').getTranslatedString('metaFields', valueInputField.value, value, service);
    ...
    const postMetaFieldsUpdate = () => {
        ...
        Object.keys(metaFieldsData).forEach(key => {
            ...
            if (Object.keys(AllowedMetaFields).includes(key)) {
                const translatedMetaFields = select('block-atfp/translate').getTranslatedString('metaFields', metaFieldsData[key], key, service);
    ...
    const postContentUpdate = () => {
        ...
        arrContent.forEach((text, index) => {
            ...
            if(text !== '' && !text.includes('atfp_skip_') && plainText){
                ...
                const translatedText = select('block-atfp/translate').getTranslatedString('content', text, uniqueKey, service);
    ...
    const updateTranslateStatus = () => {
        const ajaxUrl=window.atfp_global_object.ajax_url;
        const postId=window.atfp_global_object.current_post_id;
        const nonce=window.atfp_global_object.classic_status_key;
        const action=window.atfp_global_object.action_update_status;

        const requestBody={
            action: action,
            post_id: postId,
            status: 'completed',
            atfp_update_translation_nonce: nonce
        }
